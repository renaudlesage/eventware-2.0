...
const CRITERES = [
  { cle: 'structure.chapiteau', libelle: 'Chapiteau(x)' },
  { cle: 'structure.chapiteau_cuisine', libelle: 'Chapiteau cuisine distinct du public' },
  { cle: 'structure.podium', libelle: 'Podium / structure portante' },
  { cle: 'structure.tribune', libelle: 'Tribune ou gradins' },
  { cle: 'structure.gonflable', libelle: 'Structure gonflable' },
  { cle: 'activite.balade', libelle: 'Balade / marche / randonnée' },
  { cle: 'activite.camping', libelle: 'Camping provisoire' },
  { cle: 'activite.pyrotechnie', libelle: 'Effets pyrotechniques / feu' },
  { cle: 'activite.effets_speciaux', libelle: 'Brouillard, mousse ou stroboscope' }
]

export default function Conformite({ evenement, setMessage }) {
  ...
  async function charger() {
    const [r, i] = await Promise.all([
      ...
      supabase
        ...
        .select('*, referentiels(nom, portee)')
        .order('code')
    ])
    if (i.error) {
    ...
    const applicables = (i.data ?? []).filter((it) => {
