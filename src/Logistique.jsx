...
export default function Logistique({ evenement, membre, peut, toutPouvoir }) {
  ...
  return (
    <div className="bloc securite dom-bronze">
      ...
      {onglet === 'radio' && (
        <Radio evenement={evenement} setMessage={setMessage} peut={peut} toutPouvoir={toutPouvoir} />
      )}
      {onglet === 'jauge' && (
        <Jauge
          evenement={evenement}
...
function Stocks({ evenement, membre, peut, toutPouvoir, setMessage }) {
  const [articles, setArticles] = useState([])
  const [lieux, setLieux] = useState([])
  const [ouvert, setOuvert] = useState(null)
  const [ouvrirCreation, setOuvrirCreation] = useState(false)

  // Un article est un référentiel ; un mouvement est de la logistique.
  const peutCreerArticle = toutPouvoir || peut?.('referentiels', 'creer')
  const peutMouvement = toutPouvoir || peut?.('logistique', 'creer')
...
function Attributions({ evenement, peut, toutPouvoir, setMessage }) {
  ...
  const peutRemettre = toutPouvoir || peut?.('logistique', 'creer')
  const peutRendre = toutPouvoir || peut?.('logistique', 'modifier')
...
function Jauge({ evenement, membre, peut, toutPouvoir, setMessage }) {
  const [jauge, setJauge] = useState(null)
  const [lieux, setLieux] = useState([])
  const [lieu, setLieu] = useState('')
  const [derniers, setDerniers] = useState([])

  const peutCompter = toutPouvoir || peut?.('logistique', 'creer')
...
function Transports({ evenement, peut, toutPouvoir, setMessage }) {
  const [lignes, setLignes] = useState([])
  const [lieux, setLieux] = useState([])
  ...
  return (
    <>
      {ouvrir ? (
        ...
        <div className="ligne-boutons" style={{ marginBottom: 12 }}>
          {peutCreer && <button onClick={() => setOuvrir(true)}>Nouvelle demande</button>}
      ...
      {visibles.length === 0 ? (
        ...
        visibles.map((l) => (
          <div className="carte" key={l.id}>
            ...
            <div className="meta">
              {/* Le statut se lit ici pour qui ne peut pas le changer :
                  sans ça, cacher le sélecteur le cachait tout court. */}
              {!peutModifier && (
                <span>{STATUTS.find(([v]) => v === l.statut)?.[1] ?? l.statut}</span>
              )}
